...
class RightTriangleScreenView(Screen):
    def queans(self):
        ...
        if side_1_input != '' and side_2_input != '':
            ...
            self.ids.hypotenuse_value.hint_text = str(
                "{:.3f}".format(((float(side_1_input) ** 2) + (float(side_2_input) ** 2)) ** (1 / 2)))
            self.ids.perimeter_value.hint_text = str(
                "{:.3f}".format(float(side_1_input) + float(side_2_input) + float(self.ids.hypotenuse_value.hint_text)))
        else:
