...
class RegularHeptagonScreenView(Screen):
    def queans(self):
        ...
        if side_input != '':
            self.ids.area_value.hint_text = str("{:.3f}".format((7 / 4) * (float(side_input) ** 2) * 2.07652139657234))
            self.ids.perimeter_value.hint_text = str("{:.3f}".format(7 * float(side_input)))
        else:
