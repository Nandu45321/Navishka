...
class RegularNonagonScreenView(Screen):
    def queans(self):
        ...
        if side_input != '':
            self.ids.area_value.hint_text = str("{:.3f}".format((9 / 4) * (float(side_input) ** 2) * 2.74747741945462))
            self.ids.perimeter_value.hint_text = str("{:.3f}".format(9 * float(side_input)))
        else:
