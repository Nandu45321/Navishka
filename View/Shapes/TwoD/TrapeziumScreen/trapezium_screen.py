...
class TrapeziumScreenView(Screen):
    def queans(self):  # par1, par2, height, side1, side2
        ...
        if parallel_side_1_input != '' and parallel_side_2_input != '' and height_input != '':
            self.ids.area_value.hint_text = str("{:.3f}".format((1 / 2) * (float(parallel_side_1_input) + float(parallel_side_2_input)) * float(height_input)))
            if side_1_input != '' and side_2_input != '':
                self.ids.perimeter_value.hint_text = str("{:.3f}".format(float(parallel_side_1_input) + float(parallel_side_2_input) + float(side_1_input) + float(side_2_input)))
            else:
                self.ids.perimeter_value.hint_text = ''
                Snackbar(text="[color=#ff6961]Please fill the all the blanks for perimeter[/color]", ).open()
        else:
